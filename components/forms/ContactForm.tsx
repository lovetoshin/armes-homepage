...
function Input({
  ...
  return (
    <input
      ...
      disabled={disabled}
      className="w-full bg-white/[0.04] border border-white/[0.08] text-white placeholder-zinc-600 rounded-xl px-4 py-3 text-sm outline-none focus:border-violet-500/60 focus:bg-white/[0.06] transition-all duration-200 disabled:opacity-50"
    />
  );
}
...
interface ContactFormProps {
  onSuccess?: () => void;
  compact?: boolean;  // 모달 내부에서 사용 시 compact 모드
}
...
export default function ContactForm({ onSuccess, compact = false }: ContactFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setErrorMsg(json.message || "오류가 발생했습니다.");
        setStatus("error");
      }
    } catch {
  ...
  return (
    <form id={uid} onSubmit={handleSubmit} className="flex flex-col gap-6">

      {/* ── 문의 유형 선택 ── */}
      <div>
        <p className="text-sm font-medium text-zinc-300 mb-3">
          문의 유형<span className="text-violet-400 ml-0.5">*</span>
        </p>
        <div className={`grid gap-2 ${compact ? "grid-cols-2 sm:grid-cols-3" : "grid-cols-2 sm:grid-cols-5"}`}>
          {INQUIRY_TYPES.map((t) => (
            <button
              ...
              className={`flex flex-col gap-1 items-start p-3 rounded-xl border text-left transition-all duration-200 ${
                form.type === t.value
                  ? "bg-violet-500/12 border-violet-500/50 text-white"
                  : "bg-white/[0.03] border-white/[0.07] text-zinc-400 hover:border-white/[0.14] hover:text-zinc-200"
              }`}
            >
              <span className="text-lg leading-none">{t.icon}</span>
              <span className="text-xs font-semibold leading-tight">{t.label}</span>
              {!compact && <span className="text-[10px] text-zinc-600 leading-tight">{t.desc}</span>}
            </button>
          ))}
        </div>
      </div>
      ...
      <Field label="문의 내용" required>
        <div className="relative">
          <textarea
            ...
            rows={compact ? 4 : 5}
            disabled={isLoading}
            className="w-full bg-white/[0.04] border border-white/[0.08] text-white placeholder-zinc-600 rounded-xl px-4 py-3 text-sm outline-none focus:border-violet-500/60 focus:bg-white/[0.06] transition-all duration-200 resize-none disabled:opacity-50"
          />
      ...
      <label className="flex items-start gap-3 cursor-pointer group">
          ...
          <div
            className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-200 ${
              form.agree
                ...
                : "border-white/20 group-hover:border-white/30 bg-transparent"
            }`}
          >
            <AnimatePresence>
              {form.agree && (
        ...
        <p className="text-xs text-zinc-500 leading-relaxed">
          <span className="text-zinc-300 font-medium">개인정보 수집 및 이용에 동의합니다.</span>
          {" "}수집 항목: 이름, 이메일, 연락처 / 목적: 문의 처리 및 안내 / 보유 기간: 1년
          {" "}
          <a href="/privacy" target="_blank" className="text-violet-400 hover:underline">
            자세히 보기
          </a>
        </p>
      </label>
      ...
      <AnimatePresence>
        {(status === "error" || errorMsg) && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -8 }}
            className="flex items-center gap-2.5 p-4 rounded-xl bg-red-500/8 border border-red-500/20"
          >
      ...
      <motion.button
        ...
        className={`relative w-full py-4 rounded-2xl font-bold text-[15px] transition-all duration-300 overflow-hidden ${
          form.type && !isLoading
            ? "bg-violet-600 hover:bg-violet-500 text-white shadow-lg shadow-violet-500/20"
            : "bg-white/[0.06] text-zinc-500 cursor-not-allowed border border-white/[0.07]"
        }`}
      >
        {isLoading ? (
      ...
      <p className="text-center text-xs text-zinc-600">
        또는 바로 연락하기:{" "}
        <a href="tel:01049959867" className="text-zinc-400 hover:text-white transition-colors">
          010-4995-9867
        </a>
        {" "}·{" "}
        <a href="mailto:support@armes.co.kr" className="text-zinc-400 hover:text-white transition-colors">
          support@armes.co.kr
        </a>
      </p>
    </form>
