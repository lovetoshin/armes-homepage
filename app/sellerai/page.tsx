    ...
    <div className="min-h-screen bg-[#09090B]">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-[#09090B]/90 backdrop-blur-xl border-b border-white/[0.06]">
        <div className="max-w-6xl mx-auto px-5 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/contact"
              className="text-sm bg-violet-600 hover:bg-violet-500 text-white px-4 py-2 rounded-full font-semibold transition-all"
            >
              문의하기
            </Link>
          </div>
      ...
      <div className="pt-28 pb-20 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-5 text-center">
          ...
          {/* Token preview */}
          <div className="mt-8 inline-flex items-center gap-3 px-5 py-3 rounded-full bg-white/[0.04] border border-white/[0.08] text-sm text-zinc-400">
            <span className="text-amber-400">🪙</span>
            <span>사진 생성 <strong className="text-white">50T</strong></span>
            <span className="text-zinc-700">·</span>
            <span>상세페이지 생성 <strong className="text-white">250T</strong></span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-5 lg:px-8 pb-24">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 mb-24">
          {tools.map((tool) => (
            <div
              key={tool.title}
              className={`p-6 rounded-3xl bg-white/[0.03] border ${tool.bgColor} hover:bg-white/[0.05] transition-all duration-200 group`}
            >
              {/* Icon */}
              <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${tool.color} bg-opacity-20 flex items-center justify-center mb-4 text-2xl`}>
                {tool.icon}
              </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${tool.badgeColor}`}>
                  {tool.badge}
                </span>
              </div>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative p-7 rounded-3xl border transition-all duration-200 ${
                plan.highlight
                  ? "bg-violet-500/[0.08] border-violet-500/40 shadow-lg shadow-violet-500/10"
            ...
            >
              ...
              <Link
                href="/contact"
                className={`block text-center py-3 rounded-2xl font-semibold text-sm transition-all ${
                  plan.highlight
        ...
        <div className="text-center p-12 rounded-3xl bg-white/[0.03] border border-white/[0.07]">
          ...
          <Link
            href="/contact"
            className="inline-flex items-center gap-2 bg-white text-black px-8 py-4 rounded-full font-bold text-[15px] hover:bg-zinc-100 transition-all hover:scale-[1.02]"
          >
            얼리버드 신청하기
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
